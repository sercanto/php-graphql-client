 ...
 * @package GraphQL
 */
class Mutation extends NestableObject
{
    use FieldTrait;

    /**
     * Stores the GraphQL mutation format
     *
    ...
    /**
     * Stores the list of variables to be used in the mutation
     *
     * @var array|Variable[]
     */
    protected $variables;
    ...
    /**
     * Stores the list of arguments used when mutating data
     *
     * @var array
     */
    protected $arguments;
    ...
    /**
     * Private member that's not accessible from outside the class, used internally to deduce if mutation is nested or not
     *
     * @var bool
     */
    protected $isNested;
    ...
    /**
     * GQLMutationBuilder constructor.
     *
     * @param string $fieldName if no value is provided for the field name an empty mutation object is assumed
     * @param string $alias the alias to use for the mutation if required
     */
     ...
     */
    public function setArguments(array $arguments): Mutation
    {
        ...
        if (!empty($nonStringArgs)) {
            throw new ArgumentException(
                'One or more of the arguments provided for creating the mutation does not have a key, which represents argument name'
            );
        }
    ...
    protected function constructArguments(): string
    {
        ...
        foreach ($this->arguments as $name => $value) {
            ...
            if (is_scalar($value) || $value === null) {
                ...
                $value = StringLiteralFormatter::formatArrayForGQLMutation($value);
            }
    ...
    {
        $mutationFormat = static::QUERY_FORMAT;
        $selectionSetString = $this->constructSelectionSet();

        if (!$this->isNested) {
            $mutationFormat = $this->generateSignature();
            if ($this->fieldName === '') {

                return $mutationFormat . $selectionSetString;
            } else {
                $mutationFormat = $this->generateSignature() . " {" . PHP_EOL . static::QUERY_FORMAT . PHP_EOL . "}";
            }
        }
        $argumentsString = $this->constructArguments();

        return sprintf($mutationFormat, $this->generateFieldName(), $argumentsString, $selectionSetString);
    }
